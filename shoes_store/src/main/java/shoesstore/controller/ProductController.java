...
public class ProductController {
	...
	@GetMapping("add")
	public String formProduct(Model model, HttpSession session) {
		while(session.getAttribute("usernameAdmin") != null) {
	...
	public String addNewProduct(@ModelAttribute("product") Product product , Model model) {
		try {
			...
			System.out.println("File name null"+ file + fileName);
	...
	@PostMapping("edit/{id}")
	public String editCategory(@ModelAttribute("product") Product product , Model model, HttpServletRequest request) {
		try {
			...
			int categoryId = product.getCategory().getId();
			Category category = categoryDao.findById(Category.class,categoryId);
			System.out.println("Cate product" + product.getCategory());
			product.setCategory(category);
