...
public class LoginController {
	...
	@PostMapping("/login")
	public String login(@ModelAttribute("account") User account, Model model,HttpServletRequest request) {
		List<User> user = userDao.findByProperty("email", account.getEmail());
		BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
		
				if(user!=null && !user.isEmpty()) {
					if(encoder.matches(account.getPassword(), user.get(0).getPassword())) {
						HttpSession session1 = request.getSession();
						session1.setAttribute("customerC", user.get(0));
						//setting session to expiry in 30 mins
						session1.setMaxInactiveInterval(30*60);
						System.out.println((session1.getAttribute("customerC")).getClass());
						System.out.println(((User) session1.getAttribute("customerC")).getPassword());
